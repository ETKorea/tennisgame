...
int main()
{
    ...
    opponentScoreText.setCharacterSize(30);
    opponentScoreText.setFillColor(sf::Color::White);
    opponentScoreText.setPosition(gameWidth - 150.f, 20.f);
    opponentScoreText.setString("Opponent: " + std::to_string(opponentScore));

    // Additional variables for obstacles
    const int maxObstacles = 4;
    const float obstacleSize = 30.f;
    sf::RectangleShape obstacles[maxObstacles];
    sf::Clock obstacleTimer;
    const sf::Time obstacleInterval = sf::seconds(7.f);

    // Additional variables for ball speed boost
    sf::Clock speedBoostTimer;
    bool isSpeedBoostActive = false;
    const sf::Time speedBoostDuration = sf::seconds(3.f);
    const float boostedBallSpeed = 800.f;

    // Additional variables for item effects
    sf::Clock itemTimer;
    bool isItemActive = false;
    bool isItemApplied = false;
    const sf::Time itemDuration = sf::seconds(5.f);
    const float itemFactorOpponent = 0.5f;
    const float itemFactorPlayer = 1.5f;
    ...
    while (window.isOpen())
    {
        ...
        if (isPlaying)
        {
            ...
            if (obstacleTimer.getElapsedTime() > obstacleInterval)
            {
                obstacleTimer.restart();
                for (int i = 0; i < maxObstacles; ++i)
                {
                    if (obstacles[i].getSize().x == 0.f)
                    {
                        obstacles[i].setSize(sf::Vector2f(obstacleSize, obstacleSize));
                        obstacles[i].setFillColor(sf::Color::Red);
                        obstacles[i].setPosition(static_cast<float>(std::rand() % static_cast<int>(gameWidth - 20) + 10),
                            static_cast<float>(std::rand() % static_cast<int>(gameHeight - 20) + 10));
                        break;
                    }
                }
            }
            ...
            for (int i = 0; i < maxObstacles; ++i)
            {
                if (obstacles[i].getSize().x != 0.f)
                {
                    if (ball.getGlobalBounds().intersects(obstacles[i].getGlobalBounds()))
                    {
                        ballSound.play();
                        ballAngle = -ballAngle;
                        break;
            ...
            if (!isSpeedBoostActive && std::rand() % 300 == 0) // Randomize the timing (approximately every 10 seconds)
            {
                isSpeedBoostActive = true;
                ballSpeed *= 2.f;
                speedBoostTimer.restart();
            }
            if (speedBoostTimer.getElapsedTime() > speedBoostDuration && isSpeedBoostActive)
            {
            ...
            // Update item effects
            if (itemTimer.getElapsedTime() > itemDuration)
            {
                isItemActive = false;
                isItemApplied = false;
            }
            ...
            // Apply item effect
            if (isItemActive && !isItemApplied && playerScore >= 2)
            {
                if (std::rand() % 2 == 0)
                {
                    rightPaddleSpeed *= itemFactorOpponent;
                    isItemApplied = true;
                }
                else
                {
                    paddleSpeed *= itemFactorPlayer;
                    isItemApplied = true;
                }
            }
        }
